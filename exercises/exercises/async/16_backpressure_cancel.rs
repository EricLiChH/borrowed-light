...
async fn observe_backpressure_then_cancel() -> (bool, bool) {
    ...
    // TODO: Use tokio::select! so an immediately ready shutdown branch wins over
    // the still-pending send. Return true only when shutdown cancels the branch.
    let cancelled_by_shutdown = todo!();
